...
###################################################################################################
maximum_allowable_rows = 499999
file_counter = 0
row_counter = 0
source_dir = Path('/User/fatime/dev/csv/csvp2/')
output_dir = Path('/User/fatime/dev/csv/csvp2/output_files_mem/')
files = source_dir.glob('membership.csv')
fieldname = []

print(f"Making output directory now")
...
with open('membership.csv', 'r') as f:
    ...
    while file_counter < files_to_create:
        output_filename = f'membership_{file_counter}.csv'
